/*
------------------------------------------------------------------------------ 
This code was generated by Amplication. 
 
Changes to this file will be lost if the code is regenerated. 

There are other ways to to customize your code, see this doc to learn more
https://docs.amplication.com/how-to/custom-code

------------------------------------------------------------------------------
  */
import { ArgsType, Field } from "@nestjs/graphql";
import { ApiProperty } from "@nestjs/swagger";
import { PropertyTypeWhereInput } from "./PropertyTypeWhereInput";
import { IsOptional, ValidateNested, IsInt } from "class-validator";
import { Type } from "class-transformer";
import { PropertyTypeOrderByInput } from "./PropertyTypeOrderByInput";

@ArgsType()
class PropertyTypeFindManyArgs {
  @ApiProperty({
    required: false,
    type: () => PropertyTypeWhereInput,
  })
  @IsOptional()
  @ValidateNested()
  @Field(() => PropertyTypeWhereInput, { nullable: true })
  @Type(() => PropertyTypeWhereInput)
  where?: PropertyTypeWhereInput;

  @ApiProperty({
    required: false,
    type: [PropertyTypeOrderByInput],
  })
  @IsOptional()
  @ValidateNested({ each: true })
  @Field(() => [PropertyTypeOrderByInput], { nullable: true })
  @Type(() => PropertyTypeOrderByInput)
  orderBy?: Array<PropertyTypeOrderByInput>;

  @ApiProperty({
    required: false,
    type: Number,
  })
  @IsOptional()
  @IsInt()
  @Field(() => Number, { nullable: true })
  @Type(() => Number)
  skip?: number;

  @ApiProperty({
    required: false,
    type: Number,
  })
  @IsOptional()
  @IsInt()
  @Field(() => Number, { nullable: true })
  @Type(() => Number)
  take?: number;
}

export { PropertyTypeFindManyArgs as PropertyTypeFindManyArgs };
